...
export default function GuardianSettings() {
  ...
  useEffect(() => {
    getGuardiansForSenior(getCurrentSeniorUserId())
      .then(([first]) => setGuardian(first ?? null))
      .catch(() => setGuardian(null));
  }, []);

  return (
    <AppShell title="보호자 설정" active="guardian">
      ...
      <section className="absolute left-[68px] top-[144px] flex h-[132px] w-[1010px] items-center rounded-[8px] border border-[#d9d9d9] bg-white px-[31px]">
        {guardian ? <>
          <div className="flex h-[76px] w-[76px] items-center justify-center rounded-full border border-[#d9d9d9] bg-[#f5f5f5] text-[28px]">👨‍💼</div>
          <div className="ml-[28px]"><strong className="text-[22px]">{guardian.guardianName || guardian.guardianEmail}</strong><span className="mt-[7px] block text-[20px] text-[#6b6b6b]">{guardian.relation || "가족"}</span></div>
          <span className="ml-auto text-[30px]">›</span>
        </> : <p className="m-0 text-[20px] text-[#6b6b6b]">등록된 보호자가 없습니다</p>}
      </section>
      ...
      <section className="absolute left-[68px] top-[368px] h-[250px] w-[1010px] rounded-[8px] border border-[#d9d9d9] bg-white px-[35px] py-[30px]">
        <strong className="text-[20px] font-medium">고위험 거래 알림</strong>
        <p className="mt-[8px] text-[20px] text-[#6b6b6b]">설정한 위험도 이상인 거래가 감지되면 보호자에게 알려요.</p>
        <div className="absolute right-[62px] top-[29px]">
          {guardian && <NotificationToggle seniorUserId={guardian.seniorUserId} guardianEmail={guardian.guardianEmail} initial={guardian.alertEnabled !== false} />}
        </div>
