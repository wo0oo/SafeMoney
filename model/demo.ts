...
// I. R7 단독 발동: 평소 안 사던 업종(luxury)에서 낮 시간에 30만원 결제
//    (dailySpendAvg 5만원 × 5배 = 25만원 이상) → R7만 걸림. weight=10으로 Low 유지가 기대값 —
//    R7은 배수와 무관하게 weight가 고정이라 단독으로는 Medium을 절대 못 만든다(아래 J 참고).
run("I R7 신규업종 소비", {
  id: "I", userId: "u_01", type: "payment", amount: 300_000,
  timestamp: `${D}T14:00:00+09:00`, merchantCategory: "luxury",
}, "Low");

// J. 결제(payment) 유형의 grade 상한 확인: 심야 + 신규업종 + 500만원(threshold의 20배)이라는
//    최악의 조합이어도 R3(15)+R7(10)=25점으로 Medium 문턱(30)을 못 넘는다. 결제 유형에 걸리는
//    콤보(C1~C3)가 하나도 없어서, "결제"는 아무리 이례적이어도 시스템상 Low 이상으로 못
//    올라가는 구조적 갭 — 실서비스 적용 전 팀 논의 필요한 지점.
run("J 결제유형 grade 상한", {
